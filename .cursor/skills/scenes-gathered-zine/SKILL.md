...
description: >-
  照片 → 纸感拼贴海报（保留原始照片作为视觉锚点）。照片真实出现在最终画面中，
  配合抽象插画、手撕纸边、单色高纯度色彩构成纸感拼贴。
  Use when user mentions 实景拼贴、拾景、保留照片的海报、照片拼贴海报、
  photo collage、用这张照片做海报（保留照片）、纸感拼贴。
  前提条件：用户必须提供至少一张照片。
  不用于：无照片的纯主题海报（用 zine-poster）、不想保留照片的（用 scene-distillation-zine）。
alwaysApply: false
globs: []
---
