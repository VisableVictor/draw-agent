---
name: fireworks-diagram
description: >-
  手绘精品 SVG 技术图：软件架构图、数据流图、时序图、C4 评审、云部署图、
  事件流图、Agent/Memory 系统图、UML 类图/组件图、ER 图、网络拓扑图、
  时间线、技术概念图。12 种视觉风格，导出 SVG/PNG/GIF/交互 HTML。
  Use when user mentions 架构图、系统图、技术图、SVG图、数据流图、时序图、
  C4、云部署、UML、ER图、网络拓扑、Agent架构、拓扑图、概念图、
  生成GIF、制作GIF、SVG动画。
  不用于：数据图表、办公海报、照片处理、drawio文件。
alwaysApply: false
globs: []
---
...
This skill works in **Cursor**, **Codex**, and **Claude Code**. It follows the Agent Skills layout: `SKILL.md` is the shared entry point, bundled resources use relative paths.

Before reading a reference or running a script, resolve the directory containing the project root as `SKILL_ROOT`. In Cursor, use the git repository root or the workspace path.

**Path resolution for Cursor:**
```bash
SKILL_ROOT="$(git -C /Users/victor/IdeaProjects/draw-agent rev-parse --show-toplevel 2>/dev/null)"
```

Every command block below sets `SKILL_ROOT` itself using this pattern.
