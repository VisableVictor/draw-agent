...
description: >-
  照片 → 抽象插画海报（照片不出现在最终画面中，仅作语义灵感来源）。
  从照片中提取情绪、张力和视觉隐喻，重新创作独立的纸质插画作品。
  支持"单色块模式"触发。
  Use when user mentions 影像蒸馏、蒸馏海报、照片变插画、抽象海报、
  photo to illustration、用照片的感觉做一张新图、不要照片只要意境、单色块模式。
  前提条件：用户必须提供至少一张照片。
  不用于：想保留照片的（用 scenes-gathered-zine）、无照片输入的（用 zine-poster）。
alwaysApply: false
globs: []
---
