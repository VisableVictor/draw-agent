...
def text_box_to_rect(
    text_box: TextBox, node_id: str, pad_x: float = 0.0, pad_y: float = 0.0
) -> RectBox:
    return RectBox(
        node_id=node_id,
        x=text_box.x - pad_x,
        y=text_box.y - pad_y,
        width=text_box.width + pad_x * 2.0,
        height=text_box.height + pad_y * 2.0,
    )


def union_rects(
    boxes: list[BoxLike], node_id: str, pad_x: float = 0.0, pad_y: float = 0.0
) -> RectBox | None:
    if not boxes:
        return None
    left = min(box.x for box in boxes) - pad_x
    top = min(box.y for box in boxes) - pad_y
    right = max(box.right for box in boxes) + pad_x
    bottom = max(box.bottom for box in boxes) + pad_y
    return RectBox(node_id=node_id, x=left, y=top, width=right - left, height=bottom - top)
...
def edge_occlusion_mode(el: ET.Element) -> str:
    return (el.get("data-edge-occlusion") or "").strip()


def edge_allows_under_node_occlusion(el: ET.Element) -> bool:
    return edge_occlusion_mode(el) in {"under-node", "node-only"}
...
def collect_text_boxes_by_class(
...
def collect_page_title_boxes(root: ET.Element) -> list[TextBox]:
    return collect_text_boxes_by_class(root, "ad-title", 16.0)


def collect_page_subtitle_boxes(root: ET.Element) -> list[TextBox]:
    return collect_text_boxes_by_class(root, "ad-subtitle", 14.0)


def collect_legend_boxes(root: ET.Element) -> list[TextBox]:
    return collect_text_boxes_by_class(root, "ad-legend", 12.0)
...
def viewbox_rect(root: ET.Element) -> RectBox | None:
    values = (root.get("viewBox") or "").strip().split()
    if len(values) != 4:
        return None
    return RectBox(
        node_id="viewbox",
        x=parse_float(values[0], 0.0),
        y=parse_float(values[1], 0.0),
