...
- straight edge segments crossing diagram-box interiors
  - exception: explicit under-node edges may pass beneath node boxes only
"""
...
from svg_quality_lib import (
    ...
    boxes_overlap,
    classes_of,
    edge_allows_under_node_occlusion,
...
def collect_edge_crossings(root: ET.Element, boxes: list[NodeBox]) -> tuple[list[str], list[str]]:
    ...
    for el in root.iter():
        if not is_edge_element(el):
            continue
        allow_under_node = edge_allows_under_node_occlusion(el)
        ...
        for start, end in segments:
            for box in boxes:
                if allow_under_node and box.box_kind == "node":
                    continue
