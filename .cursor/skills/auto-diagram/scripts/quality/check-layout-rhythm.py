...
- including whether a stage/group is visibly under-filled or leaves an
  oversized blank band on one side

It exits non-zero only for usage / parse failures unless
AUTO_DIAGRAM_FAIL_ON_DESIGN_WARNING=1 or the legacy
...
def envelope_of(items: list[RectBox]) -> RectBox | None:
    if not items:
        return None
    ...
    return RectBox("envelope", left, top, right - left, bottom - top)
...
def collect_group_padding_feedback(
    ...
    for group_id, group_nodes in by_group.items():
        ...
        dominant_h_side = "right" if right > left else "left"
        dominant_h_gap = max(left, right)
        if dominant_h_gap > max(52.0, group.width * 0.16) and abs(left - right) > max(
            42.0, group.width * 0.10
        ):
            warnings.append(
                f"Group '{group_id}' leaves too much {dominant_h_side}-side blank band "
                f"({dominant_h_gap:.1f}px); expand child layout toward the {dominant_h_side} "
                "edge instead of leaving the mother container under-filled."
            )

        dominant_v_side = "bottom" if bottom > top else "top"
        dominant_v_gap = max(top, bottom)
        if dominant_v_gap > max(52.0, group.height * 0.16) and abs(top - bottom) > max(
            42.0, group.height * 0.12
        ):
            hints.append(
                f"Group '{group_id}' leaves a large {dominant_v_side} blank band "
                f"({dominant_v_gap:.1f}px); consider rebalancing children against the parent edge."
            )
...
def collect_stage_fill_feedback(
    nodes: list[NodeBox], groups: dict[str, RectBox], stages: dict[str, RectBox]
) -> tuple[list[str], list[str]]:
    warnings: list[str] = []
    hints: list[str] = []
    core_nodes = core_nodes_of(nodes)
    ...
    for stage_id, stage in stages.items():
        stage_groups = [
            ...
            if stage.contains_point(group.center_x, group.center_y, margin=0.0)
        ]
        items: list[RectBox]
        if stage_groups:
            items = stage_groups
            ...
            items = [
                ...
                if node.stage_id == stage_id or stage.contains_point(node.center_x, node.center_y)
            ]
        envelope = envelope_of(items)
        if envelope is None:
            continue

        left = envelope.x - stage.x
        right = stage.right - envelope.right
        top = envelope.y - stage.y
        bottom = stage.bottom - envelope.bottom

        width_fill = envelope.width / stage.width if stage.width > 0 else 1.0
        height_fill = envelope.height / stage.height if stage.height > 0 else 1.0

        dominant_h_side = "right" if right > left else "left"
        dominant_h_gap = max(left, right)
        if dominant_h_gap > max(72.0, stage.width * 0.08) and abs(left - right) > max(
            64.0, stage.width * 0.05
        ):
            warnings.append(
                f"Stage '{stage_id}' leaves a large {dominant_h_side}-side blank band "
                f"({dominant_h_gap:.1f}px slack); push child regions toward the {dominant_h_side} "
                "edge or shrink the stage instead of leaving the canvas under-filled."
            )

        dominant_v_side = "bottom" if bottom > top else "top"
        dominant_v_gap = max(top, bottom)
        if dominant_v_gap > max(72.0, stage.height * 0.10) and abs(top - bottom) > max(
            56.0, stage.height * 0.08
        ):
            hints.append(
                f"Stage '{stage_id}' leaves a large {dominant_v_side} blank band "
                f"({dominant_v_gap:.1f}px slack); rebalance the child envelope against the parent edge."
            )

        if width_fill < 0.72:
            warnings.append(
                f"Stage '{stage_id}' under-fills horizontal canvas: child envelope only uses "
                f"{width_fill * 100:.0f}% of stage width."
            )
        elif width_fill < 0.80:
            hints.append(
                f"Stage '{stage_id}' uses only {width_fill * 100:.0f}% of stage width; consider filling more of the canvas."
            )

        if height_fill < 0.60:
            hints.append(
                f"Stage '{stage_id}' uses only {height_fill * 100:.0f}% of stage height; consider tightening vertical slack."
            )
...
def collect_page_edge_fill_feedback(
    root: ET.Element,
    nodes: list[NodeBox],
    ...
    # First check whether the main stage itself is leaving an outer page band.
    for stage_id, stage in stages.items():
        left = stage.x - viewbox.x
        right = viewbox.right - stage.right
        top = stage.y - viewbox.y
        bottom = viewbox.bottom - stage.bottom

        dominant_h_side = "right" if right > left else "left"
        dominant_h_gap = max(left, right)
        if dominant_h_gap > max(96.0, viewbox.width * 0.08) and abs(left - right) > max(
            56.0, viewbox.width * 0.04
        ):
            warnings.append(
                f"Stage '{stage_id}' leaves a large {dominant_h_side}-side outer page band "
                f"({dominant_h_gap:.1f}px between stage and SVG edge); expand the main stage or "
                "rebalance the whole page so the canvas edge does not feel empty."
            )

        stage_width_fill = stage.width / viewbox.width
        if stage_width_fill < 0.88:
            hints.append(
                f"Stage '{stage_id}' uses only {stage_width_fill * 100:.0f}% of page width; "
                "check whether the whole composition should fill more of the horizontal canvas."
            )

        dominant_v_side = "bottom" if bottom > top else "top"
        dominant_v_gap = max(top, bottom)
        if dominant_v_gap > max(108.0, viewbox.height * 0.11) and abs(top - bottom) > max(
            72.0, viewbox.height * 0.06
        ):
            hints.append(
                f"Stage '{stage_id}' leaves a large {dominant_v_side}-side outer page band "
                f"({dominant_v_gap:.1f}px between stage and SVG edge); verify that page chrome and "
                "main content are vertically balanced."
            )

    # Then check the overall composition envelope across stage, groups, and off-stage notes.
    page_items: list[RectBox] = []
    if stages:
        page_items.extend(stages.values())
    if groups:
        page_items.extend(groups.values())
    page_items.extend(
        node
        for node in nodes
        if node.box_kind != "node" or not node.stage_id
    )

    envelope = envelope_of(page_items)
    if envelope is None:
        return warnings, hints

    left = envelope.x - viewbox.x
    right = viewbox.right - envelope.right
    top = envelope.y - viewbox.y
    bottom = viewbox.bottom - envelope.bottom

    dominant_h_side = "right" if right > left else "left"
    dominant_h_gap = max(left, right)
    if dominant_h_gap > max(88.0, viewbox.width * 0.07) and abs(left - right) > max(
        56.0, viewbox.width * 0.04
    ):
        warnings.append(
            f"Page composition leaves a large {dominant_h_side}-side blank band "
            f"({dominant_h_gap:.1f}px between the content envelope and the SVG edge); the whole "
            "picture is not visually filling the canvas evenly."
        )

    width_fill = envelope.width / viewbox.width
    if width_fill < 0.86:
        warnings.append(
            f"Page composition uses only {width_fill * 100:.0f}% of page width; inspect whether "
            "the diagram is under-filling the full SVG canvas."
        )
    elif width_fill < 0.91:
        hints.append(
            f"Page composition uses only {width_fill * 100:.0f}% of page width; consider filling "
            "more of the outer canvas if the edge balance still feels loose."
        )

    dominant_v_side = "bottom" if bottom > top else "top"
    dominant_v_gap = max(top, bottom)
    if dominant_v_gap > max(108.0, viewbox.height * 0.10) and abs(top - bottom) > max(
        72.0, viewbox.height * 0.06
    ):
        hints.append(
            f"Page composition leaves a large {dominant_v_side}-side blank band "
            f"({dominant_v_gap:.1f}px between the content envelope and the SVG edge); check the "
            "overall vertical balance of header, body, and footer."
        )
...
def main() -> int:
    ...
    stage_fill_warnings, stage_fill_hints = collect_stage_fill_feedback(nodes, groups, stages)
    warnings.extend(stage_fill_warnings)
    hints.extend(stage_fill_hints)

    stage_warnings, stage_hints = collect_stage_balance_feedback(nodes, stages)
    warnings.extend(stage_warnings)
    hints.extend(stage_hints)

    page_warnings, page_hints = collect_page_edge_fill_feedback(root, nodes, groups, stages)
    warnings.extend(page_warnings)
    hints.extend(page_hints)
