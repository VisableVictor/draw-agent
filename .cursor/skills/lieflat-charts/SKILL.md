...
name: lieflat-charts
description: >-
  数据图表专家：把数据/数字变成精美 HTML 图表（柱状图、折线图、饼图、散点图、
  雷达图、热力图等）。Lupi 编辑叙事型、Glance 快速判断型、Basics 基础型、
  Interactive 交互大图。也支持数据驱动的完整 HTML 报告（年报、月报、白皮书）。
  Use when user mentions 数据可视化、图表、chart、柱状图、折线图、饼图、
  散点图、雷达图、热力图、数据分析图、年报、月报、白皮书、
  dashboard报告、Lupi、Glance、把数据画出来。
  不用于：技术架构图、流程图、海报、照片处理。
alwaysApply: false
globs: []
---
