...
```bash
for SKILL_ROOT in \
  "$HOME/.agents/skills/draw-agent" \
  "$HOME/.claude/skills/draw-agent"
do
  [ -d "$SKILL_ROOT" ] || continue
  npm install --prefix "$SKILL_ROOT" --ignore-scripts --no-save --package-lock=false puppeteer-core@25.3.0
done
SKILL_ROOT="${CLAUDE_SKILL_DIR:-$HOME/.agents/skills/draw-agent}"
...
```bash
SKILL_ROOT=~/.agents/skills/draw-agent # Codex
# SKILL_ROOT=~/.claude/skills/draw-agent # Claude Code
...
MIT License - 与 draw-agent skill 相同
